...
def main() -> None:
    ...
    # open webcam using device index from config.
    ...
    # Initialize pygame mixer for sound (do this once, not every frame)
    pygame.mixer.init()
    SOUND_DIR = os.path.join(os.path.dirname(__file__), '..', 'assets', 'sounds')
    SLICE_SOUND = pygame.mixer.Sound(os.path.join(SOUND_DIR, 'slice.wav'))
    EXPLOSION_SOUND = pygame.mixer.Sound(os.path.join(SOUND_DIR, 'explosion.wav'))
        # SPAWN_SOUND = pygame.mixer.Sound(os.path.join(SOUND_DIR, 'spawn.wav'))
    ...
    try:
        ...
        prev_fruit_count = len(game.fruits)
        ...
        while True:
            ...
            if ui.mode == "playing":
                ...
                # Check for fruit spawn event
                prev_count = len(game.fruits)
                game.maybe_spawn_fruit(now)
                if len(game.fruits) > prev_count:
                    pass  # SPAWN_SOUND.play()
                game.update(dt)
                ...
                if len(trail) >= 2:
                    p1 = trail[-2]
                    p2 = trail[-1]
                    # Check if a bomb was sliced
                    bomb_sliced = False
                    for fruit in game.fruits:
                        if not fruit.sliced and fruit.kind == "bomb" and fruit.intersects_segment(p1, p2):
                            bomb_sliced = True
                            break
                    sliced = game.try_slice(p1, p2, now)
                    if bomb_sliced:
